...
def main(config: DictConfig):
    ...
    if config.model.archive is not None:
        state_dict = torch.load(config.model.archive,)
        
        step, metrics = state_dict['step_idx'], state_dict['metrics']
        print(f'loading pre-trained weights at step {step} from {config.model.archive} with metrics {json.dumps(metrics, indent=2)}')
        new_weights = state_dict['state']
        
        policy.load_state_dict(new_weights)
        if config.loss.name in {'dpo', 'ipo'}:
            reference_model.load_state_dict(new_weights)            
    ...
    if 'FSDP' in config.trainer:
        ...
        worker_main(world_size-1, world_size, config, policy, reference_model)
